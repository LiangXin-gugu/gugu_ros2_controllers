...
#ifndef GRIPPER_CONTROLLERS__GRIPPER_ACTION_CONTROLLER_GUGU_IMPL_HPP_
#define GRIPPER_CONTROLLERS__GRIPPER_ACTION_CONTROLLER_GUGU_IMPL_HPP_

#include "gripper_controllers_gugu/gripper_action_controller_gugu.hpp"

#include <memory>
#include <string>

namespace gripper_action_controller_gugu
{
...
{
  // RCLCPP_INFO_STREAM(get_node()->get_logger(), "gugu: update stage " << rclcpp::Clock{RCL_STEADY_TIME}.now().seconds());
  ...
  check_for_success(rclcpp::Clock{RCL_ROS_TIME}.now(), error_position, current_position, current_velocity);
...
{
  RCLCPP_INFO(get_node()->get_logger(), "gugu: action->accepted_callback");
  ...
  last_movement_time_ = rclcpp::Clock{RCL_ROS_TIME}.now();
  rt_goal->execute();
  ...
  double current_velocity)
{
  ...
  else
  {
    ...
    {
      RCLCPP_INFO(get_node()->get_logger(), "gugu: stalled");
      pre_alloc_result_->effort = computed_command_;
      pre_alloc_result_->position = current_position;
      pre_alloc_result_->reached_goal = false;
  ...
  const rclcpp_lifecycle::State &)
{
  RCLCPP_INFO(
    get_node()->get_logger(),
    "gugu: on_activate stage");

  // 【gugu】step1: checking the command_interface and state_interface
  ...
  // 【gugu】step2: command_interface and state_interface
  ...
  // 【gugu】step3: command_interface adapter
  // Hardware interface adapter
  hw_iface_adapter_.init(joint_command_interface_, get_node());

  // 【gugu】step4: the input of the controller
  // Command - non RT version
  command_struct_.position_ = joint_position_state_interface_->get().get_value();
  command_struct_.max_effort_ = params_.max_effort;
  command_.initRT(command_struct_);

  // 【gugu】step5: the execute result of the action
  // Result
  ...
  // 【gugu】step6: instantiates action server
  // Action interface
...
GripperActionController<HardwareInterface>::command_interface_configuration() const
{
  RCLCPP_INFO(
    get_node()->get_logger(),
    "gugu: command_interface_configuration stage");
  return {
...
GripperActionController<HardwareInterface>::state_interface_configuration() const
{
  RCLCPP_INFO(
    get_node()->get_logger(),
    "gugu: state_interface_configuration stage");
  return {
    controller_interface::interface_configuration_type::INDIVIDUAL,
...
#endif  // GRIPPER_CONTROLLERS__GRIPPER_ACTION_CONTROLLER_GUGU_IMPL_HPP_
