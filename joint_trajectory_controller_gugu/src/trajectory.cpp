  ...
  TrajectoryPointConstIter & start_segment_itr, TrajectoryPointConstIter & end_segment_itr)
{
  ...
  //当前轨迹为空，则采样失败（应该不会发生此情况，因为调用该函数的外层有if (has_active_trajectory())条件）
  ...
  //记录轨迹首次采样标志（针对每个轨迹指令仅在首次进入时执行一次）
  //记录轨迹起始执行时间（当轨迹指令的headerstamp为0，即表示从当前时刻开始时）
  ...
  //当前时间若小于轨迹首次采样时间，则采样失败（应该不会发生此情况，因为调用该函数之前已通过traj_external_point_ptr_->set_point_before_trajectory_msg函数设置了轨迹首次采样时间）
  ...
  // 情况1：若当前时间小于轨迹中第一个点的时间戳，则在当前点state_before_traj_msg_与第一个点first_point_in_msg之间插值得到当前指令output_state；
  ...
  // 情况2：若当前时间大于等于轨迹中第一个点，且小于轨迹中最后一个点的时间戳，则在对应估计的轨迹段内插值得到当前指令output_state；
  ...
  // 情况3：若当前时间大于等于轨迹中最后一个点的时间戳，则以轨迹最后一个点保持作为output_state持续保持；
