...
#include "joint_trajectory_controller_gugu/joint_trajectory_controller_gugu.hpp"
...
#include "joint_trajectory_controller_gugu/trajectory.hpp"
...
namespace joint_trajectory_controller_gugu
{
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  {
    fill_partial_goal(*new_external_msg);//当收到的轨迹信息的关节维数小于控制器负责的关节维数（在ros2_controller.yaml中规定）时，补全漏掉的关节维数的指令，位置为上一拍指令位置或当前位置，速度加速度为0。
    sort_to_local_joint_order(*new_external_msg);
    // TODO(denis): Add here integration of position and velocity
    traj_external_point_ptr_->update(*new_external_msg);
    
    // record new_external_msg update time to file
    {
      std::ofstream file1("/home/gugu/gugu_workspace/controllerNewExternalTrajectoryUpdateTime.csv",std::ios::app);
      file1 << time.seconds() << std::endl;
      file1.close();
    }
  }
  ...
  if (has_active_trajectory())
  {
    ...
    if (!traj_external_point_ptr_->is_sampled_already())
    {
      ...
      // Output controller current active trajectory to file (at first sample time);
      {
        auto update_external_msg = traj_external_point_ptr_->get_trajectory_msg();
        std::ofstream file1("/home/gugu/gugu_workspace/controllerCurrentActiveTrajectory.csv",std::ios::app);
        unsigned int num_joints = update_external_msg->joint_names.size();
        unsigned int num_trajectory_points = update_external_msg->points.size();

        // Output trajectory points data
        if(num_trajectory_points==1) //TODO: figure out where is this kind of trajectory from?
        {
          file1 << time.seconds() << ", ";
          double time_from_start_nanosec = (double)update_external_msg->points[0].time_from_start.nanosec/1e9;
          double time_from_start_sec = (double)update_external_msg->points[0].time_from_start.sec;
          file1 << time_from_start_sec + time_from_start_nanosec << ", ";
          for (unsigned int j = 0; j < num_joints; ++j)
          {
            file1 << update_external_msg->points[0].positions[j] << ", ";
          }
          for (unsigned int j = 0; j < num_joints; ++j)
          {
            file1 << -1 << ", ";
        ...
        else if(num_trajectory_points>2)
        {
          for (unsigned int p = 0; p < num_trajectory_points; ++p)
          {
            file1 << time.seconds() << ", ";
            double time_from_start_nanosec = (double)update_external_msg->points[p].time_from_start.nanosec/1e9;
            double time_from_start_sec = (double)update_external_msg->points[p].time_from_start.sec;
            file1 << time_from_start_sec + time_from_start_nanosec << ", ";
            
            for (unsigned int j = 0; j < num_joints; ++j)
            {
              file1 << update_external_msg->points[p].positions[j] << ", ";
            }
            for (unsigned int j = 0; j < num_joints; ++j)
            {
              file1 << update_external_msg->points[p].velocities[j] << ", ";
            }
            for (unsigned int j = 0; j < num_joints; ++j)
            {
              file1 << update_external_msg->points[p].accelerations[j] << ", ";
            }
            file1 << std::endl;
          }
        }    
    ...
    if (valid_point)
    {
      ...
      // Output controller real time state to file;
      {
        std::ofstream file1("/home/gugu/gugu_workspace/controllerRealTimeState.csv", std::ios::app);
        ...
        {
          file1 << state_desired_.positions[j] << ", ";
        }
        // for (unsigned int j = 0; j < dof_; ++j)
        // {
        //   file1 << state_desired_.velocities[j] << ", ";
        // }
        // for (unsigned int j = 0; j < dof_; ++j)
        // {
        //   file1 << state_desired_.accelerations[j] << ", ";
        // }

        for (unsigned int j = 0; j < dof_; ++j)
        {
          file1 << state_current_.positions[j] << ", ";
        }

        for (unsigned int j = 0; j < dof_; ++j)
        {
          file1 << state_error_.positions[j] << ", ";
        }
        ...
        if (read_commands_from_command_interfaces(command_current_))
        {
          for (unsigned int j = 0; j < dof_; ++j)
          {
            file1 << command_current_.positions[j] << ", ";
          }
        }
      ...
      if (active_goal)
      {
        ...
        else if (!before_last_point)
        {
          if (!outside_goal_tolerance)
          {
            ...
            RCLCPP_INFO(logger, "gugu: Goal reached, success!");
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  //Output controller received goal to file;
  {
    std::ofstream file1("/home/gugu/gugu_workspace/controllerReceivedTrajectory.csv");
  }

  // record new_external_msg update time to file
  {
    std::ofstream file1("/home/gugu/gugu_workspace/controllerNewExternalTrajectoryUpdateTime.csv");
    file1  << "rclcppTime " << std::endl;
    file1.close();
  }

  //Output controller current active trajectory to file;
  {
    std::ofstream file1("/home/gugu/gugu_workspace/controllerCurrentActiveTrajectory.csv");
    file1  << "rclcppTime, " << "time_from_start, ";
    for (unsigned int j = 0; j < dof_; ++j)
    {
      file1 << "joint_" << j+1 <<"_position"<< ", ";
    }
    for (unsigned int j = 0; j < dof_; ++j)
    {
      file1 << "joint_" << j+1 <<"_velocity"<< ", ";
    }
    for (unsigned int j = 0; j < dof_; ++j)
    {
      file1 << "joint_" << j+1 <<"_acceleration"<< ", ";
    }
    file1 << std::endl;
    file1.close();
  }
  ...
  //Output controller real time state to file;
  {
    // Output controller real time state header to file;
    std::ofstream file1("/home/gugu/gugu_workspace/controllerRealTimeState.csv");

    file1 << "rclcppTime, ";
    for (unsigned int j = 0; j < dof_; ++j)
    {
      file1 << "joint_" << j+1 <<"_desired_position"<< ", ";
    }
    for (unsigned int j = 0; j < dof_; ++j)
    {
      file1 << "joint_" << j+1 <<"_current_position"<< ", ";
    }
    for (unsigned int j = 0; j < dof_; ++j)
    {
      file1 << "joint_" << j+1 <<"_position_error"<< ", ";
    }
    for (unsigned int j = 0; j < dof_; ++j)
    {
      file1 << "joint_" << j+1 <<"_cmd_position"<< ", ";
    }
    file1 << std::endl;
    file1.close();
  }
  ...
  const std::shared_ptr<trajectory_msgs::msg::JointTrajectory> msg)
{
  ...
  // http://wiki.ros.org/joint_trajectory_controller_gugu/UnderstandingTrajectoryReplacement
...
{
  RCLCPP_INFO(get_node()->get_logger(), "gugu: Received new action goal");
  ...
  RCLCPP_INFO(get_node()->get_logger(), "gugu: Accepted new action goal");
  ...
  std::shared_ptr<rclcpp_action::ServerGoalHandle<FollowJTrajAction>> goal_handle)
{
  ...
  // Update new trajectory
  {
    ...
    // Output controller reveived trajectory to file;
    {
      std::ofstream file1("/home/gugu/gugu_workspace/controllerReceivedTrajectory.csv",std::ios::app);
      unsigned int num_joints = traj_msg->joint_names.size();
      unsigned int num_trajectory_points = traj_msg->points.size();
      // Output header
      file1  << "trajectoryHeaderStamp, "  << "time_from_start, ";
      for (unsigned int j = 0; j < num_joints; ++j)
      {
        file1 << traj_msg->joint_names[j] <<"_position"<< ", ";
      }
      for (unsigned int j = 0; j < num_joints; ++j)
      {
        file1 << traj_msg->joint_names[j] <<"_velocity"<< ", ";
      }
      for (unsigned int j = 0; j < num_joints; ++j)
      {
        file1 << traj_msg->joint_names[j] <<"_acceleration"<< ", ";
      }
      file1 << std::endl;
      ...
      for (unsigned int p = 0; p < num_trajectory_points; ++p)
      {
        rclcpp::Time tempTime = static_cast<rclcpp::Time>(traj_msg->header.stamp);
        file1 << tempTime.seconds() << ", ";
        
        double time_from_start_nanosec = (double)traj_msg->points[p].time_from_start.nanosec/1e9;
        double time_from_start_sec = (double)traj_msg->points[p].time_from_start.sec;
        file1 << time_from_start_sec + time_from_start_nanosec << ", ";
        
        for (unsigned int j = 0; j < num_joints; ++j)
        {
          file1 << traj_msg->points[p].positions[j] << ", ";
        }
        for (unsigned int j = 0; j < num_joints; ++j)
        {
          file1 << traj_msg->points[p].velocities[j] << ", ";
        }
        for (unsigned int j = 0; j < num_joints; ++j)
        {
          file1 << traj_msg->points[p].accelerations[j] << ", ";
        }
        file1 << std::endl;
      }
      file1.close();
...
PLUGINLIB_EXPORT_CLASS(
  joint_trajectory_controller_gugu::JointTrajectoryController, controller_interface::ControllerInterface)
