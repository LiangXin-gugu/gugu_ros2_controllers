...
using namespace joint_trajectory_controller_gugu::interpolation_methods;  // NOLINT
...
TEST(TestTrajectory, initialize_trajectory)
{
  auto clock = rclcpp::Clock(RCL_STEADY_TIME);
  {
    ...
    auto traj = joint_trajectory_controller_gugu::Trajectory(empty_msg);
    EXPECT_EQ(0, traj.last_sample_index());

    trajectory_msgs::msg::JointTrajectoryPoint expected_point;
    joint_trajectory_controller_gugu::TrajectoryPointConstIter start, end;
...
TEST(TestTrajectory, sample_trajectory_positions)
{
  ...
  auto traj = joint_trajectory_controller_gugu::Trajectory(time_now, point_before_msg, full_msg);
...
TEST(TestTrajectory, interpolation_pos_vel)
{
  ...
  auto traj = joint_trajectory_controller_gugu::Trajectory();
...
TEST(TestTrajectory, update_trajectory)
{
  ...
  const rclcpp::Time time_now = rclcpp::Clock().now();
  auto traj = joint_trajectory_controller_gugu::Trajectory(time_now, point_before_msg, first_msg);
  EXPECT_EQ(0, traj.last_sample_index());
...
TEST(TestWrapAroundJoint, no_wraparound)
{
  ...
  joint_trajectory_controller_gugu::wraparound_joint(
